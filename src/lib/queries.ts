...
export async function fetchMarket(page: number, limit: number) {
...
export async function fetchOrdersWithToken(page: number, limit: number, token: string) {
...
export async function fetchOrders(page: number, limit: number) {
    const res = await fetch(`http://localhost:3000/api/orders?page=${page}&limit=${limit}`, {
        credentials: "include"
    })
    if (!res.ok) {
